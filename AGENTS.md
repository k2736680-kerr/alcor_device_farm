...
9. 设备农场内部能力以 `app_evaluation_device_farm_design.md` 为基线；Alcor 平台业务对象、接口、存储、评估前端和集成边界以 `alcor_next_generation_plan.txt` 为最新基线。设备农场独立控制后台按 `docs/adr/0009_device_farm_control_console.md` 实施；其他冲突按 `docs/03_alcor_generation_alignment.md` 和 ADR-0004 处理，禁止继续沿用旧 Alcor 假设。
10. 本仓库不得发展成第二套评估平台、第二套 Eval Console 或第二套 Run/Result 数据库；允许建设只操作 Device、Image、Host、Pool、Reservation、设备域审计和 STF 受控入口的 Device Farm Console，设备域仍通过新版 Worker 的 Device Farm Adapter 接入。
