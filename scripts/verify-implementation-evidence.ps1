...
$TaskIDs = 3..25 | ForEach-Object { "DF-{0:D3}" -f $_ }
$AllowedStatuses = @("completed", "blocked")
...
$ManifestStatuses = Read-TaskStatuses -Document $Manifest `
    -Pattern '^\|\s*(DF-\d{3})\s*\|\s*(completed|blocked)\s*\|' `
...
foreach ($TaskID in $TaskIDs) {
    ...
    if ($Status -notin $AllowedStatuses) {
        throw "$TaskID cannot pass the evidence gate while status is $Status"
    }
    ...
    $HasBlockedStatus = [regex]::IsMatch($Evidence, 'blocked', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
