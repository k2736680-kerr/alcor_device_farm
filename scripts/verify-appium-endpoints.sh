...
if [ "$(uname -s)" != "Linux" ]; then
  echo "DF-015 requires a Linux host" >&2
  exit 1
fi
...
: "${DEVICE_FARM_DOCKER_ADVERTISE_HOST:?set DEVICE_FARM_DOCKER_ADVERTISE_HOST to the Host address reachable by Appium clients}"
