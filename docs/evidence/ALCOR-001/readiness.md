...
- 一级菜单展开运行概览、宿主机、设备池、设备、预约、应用版本、健康事件和操作审计八个二级入口；
- 设备域二级入口通过 Alcor 同源代理按路由嵌入既有 Device Farm Console，嵌入时隐藏其独立侧栏和页头；
- “应用版本”直接使用 Alcor 原生 APK 表格和上传入口，APK 仍由 Alcor 管理；
...
| 设备农场二级导航 | PASS | Edge 151 真实页面逐项打开运行概览、宿主机、设备池、设备、预约、健康事件和操作审计，对应代理路由均正确；嵌入内容无第二套深色侧栏 |
| APK 二级入口 | PASS | “应用版本”直接显示 Alcor 原生 APK 表格和“上传 APK”按钮，不再显示旧“设备管理 / 应用版本（APK）”切换卡 |
| 页面去重 | PASS | 设备农场页面不再显示顶部操作人筛选、重复刷新、说明及独立 Console 导航；操作人仍由已验证 Alcor 会话透传并写入设备域审计 |
...
- Device Farm Console：7 个测试文件、31 个测试全部通过，生产构建通过；
- OpenAPI 1.9.0：契约路径、冻结哈希和 Orval 生成通过；
- Alcor Server：`go test ./...` 全部通过；
- Alcor Console：生产构建和 lint 通过；lint 仅保留既有 React Hooks 警告；
- Edge 151 无头浏览器：八个二级入口、无二次登录、路由切换、嵌入侧栏隐藏和 Alcor 原生 APK 页面通过；
- Linux 测试服务：`/readyz` 返回 ready，既有 Android Emulator 容器启动时间未改变。

## 部署边界

本次只更新本地 Alcor 测试程序和现有 Device Farm 测试服务用于验收，没有推送 Git 远端、没有合并 Alcor 主分支、没有发布正式环境。测试 Device Farm 已切换到镜像 `alcor-device-farm:alcor-nav-20260813`，原容器保留为 `alcor-device-farm-server-df017-before-nav-20260813`。正式发布时只需部署这两个已提交版本并配置同一 Device Farm Endpoint 与 Service Token 引用，无数据库 migration。
