...
- `internal/management`：资源 Service、领域状态机调用、Mock Device 准备和 Provider 补偿；
- `internal/management/postgres`：四类资源 Store、通用创建幂等、Pool 成员和可调度查询；
- Server 在配置 `DEVICE_FARM_DATABASE_URL` 后自动连接 PostgreSQL 并装配 Mock Provider；未配置时管理接口返回 503，不会使用内存数据冒充持久化；
...
5. 查询 Device，执行 restart、quarantine、rebuild、unquarantine；
