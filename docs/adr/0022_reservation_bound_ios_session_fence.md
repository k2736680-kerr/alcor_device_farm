...
9. Reconciler 比对 active Reservation、技术 Session 绑定和 Agent 上报的 `providerBusy`。`providerBusy` 是技术占用事实，不是 Router 健康失败：已绑定 Session 的设备保持 `busy/healthy`；无 Reservation 的 busy、绑定 Session 的非 busy、无绑定 Session 的异常 busy 或 Host/UDID 漂移才会阻止新 Grant 并把 Device 标记 degraded/quarantined。Grant 消费成功后允许最多 300 秒完成 XCUITest/WDA 冷启动和 Session 绑定，避免现场编译期间被误隔离；超过该窗口仍未绑定才判定为漂移。Session 正常结束后的 30 秒内允许 Agent 心跳刷新插件 busy；若旧心跳在下一预约开始后才到达，Grant/Consume 返回可重试的收敛中错误但不隔离设备，超过宽限仍 busy 才按漂移隔离。插件 busy 不能自行创建、续租或释放 Reservation。
